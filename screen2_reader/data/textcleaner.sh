...
# -g                        /usr/local/Cellar/imagemagick/6.9.5-5/bin/convert document to grayscale before enhancing
...
# 2) optional crop, 
# 3) optional /usr/local/Cellar/imagemagick/6.9.5-5/bin/convert to grayscale, 
# 4) optional enhance, 
...
# 
# -g ... /usr/local/Cellar/imagemagick/6.9.5-5/bin/convert the document to grayscale.
# 
...
gray="no"			# /usr/local/Cellar/imagemagick/6.9.5-5/bin/convert to grayscale flag
...
# get im version
im_version=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert -list configure | \
...
if [ "$im_version" -ge "07000000" ]; then
	hdri_on=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert -version | grep "HDRI"`	
else
	hdri_on=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert -list configure | grep "enable-hdri"`
fi
...
# then -separate /usr/local/Cellar/imagemagick/6.9.5-5/bin/converted to linear gray channels between 6.7.6.7 and 6.7.8.2,
# though probably not resolved until the latter
...
/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert -quiet "$infile" +repage "$tmpA1" ||
	errMsg "--- FILE $infile NOT READABLE OR HAS ZERO SIZE ---"

# get image size
ww=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert $tmpA1 -ping -format "%w" info:`
hh=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert $tmpA1 -ping -format "%h" info:`

# get image h/w aspect ratio and determine if portrait=1 (h/w>1) or landscape=0 (h/w<1)
aspect=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert xc: -format "%[fx:($hh/$ww)>=1?1:0]" info:`
...
if [ "$cropoff" != "" -a $numcrops -eq 1 ]; then
	wwc=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert xc: -format "%[fx:$ww-2*$crop1]" info:`
	hhc=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert xc: -format "%[fx:$hh-2*$crop1]" info:`
	cropping="-crop ${wwc}x${hhc}+$crop1+$crop1 +repage"
elif [ "$cropoff" != "" -a $numcrops -eq 2 ]; then
	wwc=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert xc: -format "%[fx:$ww-2*$crop1]" info:`
	hhc=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert xc: -format "%[fx:$hh-2*$crop2]" info:`
	cropping="-crop ${wwc}x${hhc}+$crop1+$crop2 +repage"
elif [ "$cropoff" != "" -a $numcrops -eq 4 ]; then
	wwc=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert xc: -format "%[fx:$ww-($crop1+$crop3)]" info:`
	hhc=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert xc: -format "%[fx:$hh-($crop2+$crop4)]" info:`
	cropping="-crop ${wwc}x${hhc}+$crop1+$crop2 +repage"
else
	cropping=""
fi
...
# test if grayscale
grayscale=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert $tmpA1 -format "%[colorspace]" info:`
typegray=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert $tmpA1 -format '%r' info: | grep 'Gray'`
...
#	bluramt=`/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert xc: -format "%[fx:$threshold/100]" info:`
...
# process image
/usr/local/Cellar/imagemagick/6.9.5-5/bin/convert -respect-parenthesis \( $tmpA1 $rotation $cropping $makegray $enhancing \) \
